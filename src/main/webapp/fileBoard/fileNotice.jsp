				...
				<div class="content">
					...
					<table>
						...
						if (totalCount > 0) {
							...
							for (BoardVo vo : list) {
								...
								<tr>
									<td><%=vo.getNum() %></td>
									<td class='left' onclick="location.href='fileContent.jsp?num=<%=vo.getNum() %>&pageNum=<%=pageNum %>&category=<%=category %>&search=<%=search %>'">
										<%
					...
					if (totalCount > 0) {
						...
						for (int i=startPage; i<=endPage; i++) {
							%>
							<a href="fileNotice.jsp?pageNum=<%=i %>&category=<%=category %>&search=<%=search %>">
							<%
							if (i == pageNum) {
								%>
						...
						if (endPage < pageCount) {
							%>
							<a href="fileNotice.jsp?pageNum=<%=endPage + 1 %>&category=<%=category %>&search=<%=search %>">[다음]</a>
							<%
						}
					}
					%>
					...
					<div class="row unifrom">
						...
						<div class="6u 12u$(small)" >
							<ul class="actions small align-right" >
								<li><input type="button" value="글쓰기" class="btn button special small" onclick="location.href='fileWriteForm.jsp?pageNum=<%=pageNum %>&category=<%=category %>&search=<%=search %>'"></li>
							</ul>
						</div>
					</div>	
					<div class="clear"></div>
