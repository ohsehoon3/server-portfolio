		...
		<div class="inner">
			<div class="box">
				<div class="content">
					...
					<form action="deleteContents.jsp?category=<%=category %>&search=<%=search %>" method="post">
						<table>
							<thead>
								<tr>
									<th> </th>
								...
								if (totalCount > 0) {
									...
									for (BoardVo vo : list) {
										...
										<tr>
											...
											<td><%=vo.getNum() %></td>
											<td class='left' onclick="window.open('/board/content.jsp?num=<%=vo.getNum() %>&pageNum=<%=pageNum %>&category=<%=category %>&search=<%=search %>')">
												<%
											...
											<td><%=vo.getIp() %></td>
											<td><input type="button" value="수정" onclick="window.open('/board/updateWriteForm.jsp?category=<%=category %>&search=<%=search %>&pageNum=<%=pageNum %>&num=<%=vo.getNum() %>')" class="btn button alt small"></td>
										</tr>
										<%
									}
								} else {
					...
					if (totalCount > 0) {
						...
						if (startPage > pageBlock) { // 여러가지 방법 가능
							%>
							<a href="adminNotice.jsp?pageNum=<%=startPage - pageBlock %>&category=<%=category %>&search=<%=search %>">[이전]</a>&nbsp;&nbsp;
							<%
						}
						...
						for (int i=startPage; i<=endPage; i++) {
							%>
							<a href="adminNotice.jsp?pageNum=<%=i %>&category=<%=category %>&search=<%=search %>">
							<%
							if (i == pageNum) {
								%>
						...
						if (endPage < pageCount) {
							%>
							<a href="adminNotice.jsp?pageNum=<%=endPage + 1 %>&category=<%=category %>&search=<%=search %>">[다음]</a>
							<%
						}
					}
					%>
					...
					<div class="row unifrom">
						<div id="table_search" class="6u 12u$(small)">
							<form action="adminNotice.jsp" method="get">
								<ul class="actions small">
									<li>
						...
						<div class="6u 12u$(small)" >
							<ul class="actions small align-right">
								<li><input type="button" value="글쓰기" class="btn button special small" onclick="window.open('/board/writeForm.jsp?pageNum=<%=pageNum %>&category=<%=category %>&search=<%=search %>')"></li>
							</ul>
						</div>
					</div>	
					<div class="clear"></div>
